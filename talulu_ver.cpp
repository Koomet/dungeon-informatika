...
void openChestZone5(Player &player);
...
void bossBattle_ForestGuardian(Player &player);
void act0();
...
struct Player
{
    ...
    int exp = 0;
    int gold = 0;
    int deflectPct = 0;   // persentase damage deflect
    int lifestealPct = 0; // persentase lifesteal
    Inventory inv;
};
Enemy slime = {"Slug", 25, 25, 5, 1, 1, 5, 5, "T1"};
Enemy goblin = {"Goblin", 40, 40, 8, 2, 4, 15, 15, "T1"};
Enemy skeleton = {"Skeleton", 60, 60, 10, 5, 3, 20, 20, "T2"};
Enemy darkWolf = {"Dark Wolf", 90, 90, 15, 6, 6, 35, 35, "T3"};
Enemy cursedKnight = {"Cursed Knight", 140, 140, 22, 10, 5, 60, 60, "T4"};
Enemy slug1 = {"Slug", 25, 25, 5, 1, 1, 5, 5, "T1"};
Enemy slug2 = {"Infected slug", 30, 30, 7, 1, 1, 10, 10, "T1"};
Enemy wolf = {"Wolf", 40, 40, 8, 2, 4, 15, 15, "T1"};
Enemy wolf2 = {"Infected Wolf", 50, 45, 10, 2, 4, 25, 25, "T1"};
Enemy spider = {"Spider", 35, 30, 15, 5, 3, 20, 20, "T2"};
Enemy skeleton2 = {"Crystallized Skeleton", 65, 80, 11, 5, 3, 30, 30, "T2"};
Enemy darkorb = {"Dark Orb", 90, 90, 15, 6, 6, 35, 35, "T3"};
Enemy darkorb2 = {"Crystal Orb", 100, 100, 17, 6, 6, 45, 45, "T3"};
Enemy livingarmor = {"Living Armor", 140, 140, 22, 10, 5, 60, 60, "T4"};
Enemy livingarmor2 = {"Living Crystal", 150, 160, 25, 10, 5, 80, 80, "T4"};
// Enemy slime        = {"Infected Slug", 30, 30, 7, 1, 1, 5, "T1"};
// Enemy slime        = {"Slime", 35, 30, 7, 1, 1, 5, "T1"};
// Enemy slime        = {"Slime", 25, 25, 5, 1, 1, 5, "T1"};
// Enemy slime        = {"Slime", 25, 25, 5, 1, 1, 5, "T1"};
Enemy INFORMATIKAJAHAT = {"INFORMATIKA JAHAT", 300, 300, 15, 10, 8, 200, 250, "T5"};
Enemy treant = {"Treant", 120, 120, 18, 8, 4, 50, 50, "T3"};
Enemy darkElf = {"Dark Elf", 160, 160, 25, 12, 8, 80, 80, "T4"}; // nabil hate dark elf, racist
Enemy giantSpider = {"Giant Spider", 130, 130, 30, 5, 7, 70, 70, "T4"};
Enemy shadowBeast = {"Shadow Beast", 200, 200, 20, 15, 10, 150, 150, "T5"};
...
int playerAttack(Player &p, Enemy &e)
{
    ...
    e.hp -= dmg;
    if (p.lifestealPct > 0 && dmg > 0)
    {
        int heal = (dmg * p.lifestealPct) / 100;
        p.hp = min(p.maxHP, p.hp + heal);
        narrate("Lifesteal aktif! Kamu memulihkan " + to_string(heal) + " HP.");
    }
...
int enemyAttack(Player &p, Enemy &e)
{
    ...
    p.hp -= dmg;
    // deflect mechanic (untuk armor dan semacamnya)
    // pls no bug plss
    if (p.deflectPct > 0)
    {
        int reflected = (dmg * p.deflectPct) / 100;
        e.hp -= reflected;
        narrate("Damage dipantulkan! Musuh menerima " + to_string(reflected) + " damage.");
    }
    return dmg;
}
...
void merchantRestArea(Player &player) {
    narrate("\n=== Rest Area Merchant ===");
    narrate("A mysterious hooded merchant sits by the fire.");
    dialog("Merchant:", "I've got some rare items... if you have enough golds.");

    while (true) {
        cout << "\nYour Gold: " << player.gold << endl;
        cout << "\n===== RARE SHOP =====" << endl;
        cout << "1. Rune Blade (80 gold) - 50 True Damage" << endl;
        cout << "2. Smoke Orb (50 gold)  - 100% Escape" << endl;
        cout << "3. Vampiric Fang (120 gold) - Lifesteal +20%" << endl;
        cout << "4. Spike Shield (90 gold) - Reflect 25% damage" << endl;
        cout << "5. Leave Shop" << endl;

        int c = chooseOption(1, 5);

        // RUNE BLADE
        if (c == 1) {
            if (player.gold < 80) narrate("You can't afford this.");
            else {
                player.gold -= 80;
                addItem(player.inv, "Rune Blade", 1, "50 True Damage");
                narrate("You bought Rune Blade.");
            }
        }

        // SMOKE ORB
        else if (c == 2) {
            if (player.gold < 50) narrate("Not enough gold.");
            else {
                player.gold -= 50;
                addItem(player.inv, "Smoke Orb", 1, "100% Escape");
                narrate("You bought Smoke Orb.");
            }
        }

        // VAMPIRIC FANG
        else if (c == 3) {
            if (player.gold < 100) narrate("Too expensive for you.");
            else {
                player.gold -= 100;
                addItem(player.inv, "Vampiric Fang", 1, "Lifesteal +20%");
                narrate("You bought Vampiric Fang.");
            }
        }

        // SPIKE SHIELD
        else if (c == 4) {
            if (player.gold < 90) narrate("Need more gold.");
            else {
                player.gold -= 90;
                addItem(player.inv, "Spike Shield", 1, "Deflect 25%");
                narrate("You bought Spike Shield.");
            }
        }

        else if (c == 5) {
            narrate("You leave the rare merchant.");
            break;
        }
    }
}
...
void zone4(Player &player)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        {
            narrate("You keep walking through dungeon.");
            narrate("holding your weapons tightly");
            narrate("Moving along.");
            pressEnter();
        }
    }
...
{
    narrate("=== Zona 5: Dark Underground Forest ===");
    showHUD(player);
    dialog(player.name, ": This forest feels alive... and not in a good way.");
    dialog(player.name, ": The trees seem to whisper, and the shadows move on their own.");
    narrate("You venture deeper into the eerie forest...");
    ...
    for (int i = 0; i < 7; i++)
    {
        ...
        {
            showHUD(player);
            openChestZone5(player);
            pressEnter();
        }
        else if (roll <= 85)
        {
            showHUD(player);
            vector<Enemy> pool = {treant, darkElf, giantSpider, shadowBeast};
            vector<int> chance = {30, 25, 25, 20};
            randomEncounter(player, pool, chance);
            pressEnter();
        }
...
void openChestZone4(Player &player)
{
    int loot = rng(1, 100);

    if (loot <= 20)
        addItem(player.inv, "Spike Shield", 1, "25% deflect damage");
    else
        addItem(player.inv, "Potion", 1, "Heal 40 HP");
}
...
void openChestZone5(Player &player)
{
    ...
    {
        addItem(player.inv, "Mystic Herb", 1, "Heal 60 HP and remove poison");
        narrate("You found Mystic Herb!");
    }
    else if (roll <= 95)
    {
        player.maxHP += 75;
        narrate("You found Ancient Crystal! Max HP +75!");
    }
    else
    {
        player.attack += 30;
        narrate("You found Enchanted Blade! Attack +30 permanent!");
    }
}
...
bool useItemInBattle(Player &player, Enemy &enemy, const string &itemNameInput)
{
    ...
    for (auto &it : player.inv.items)
    {
        if (toLowerTrim(it.name) == key && it.quantity > 0)
        {
            ...
            else if (toLowerTrim(it.name) == "Rune Blade")
            {
                int dmg = 50;
                enemy.hp -= dmg;
                narrate("Rune Blade! TRUE DAMAGE " + to_string(dmg) + "!");
                it.quantity--;
            }
            else if (toLowerTrim(it.name) == "Vampiric Fang")
            {
                player.lifestealPct = 20;
                narrate("Kamu merasa energi mengalir... Lifesteal +20%!");
                it.quantity--;
            }
            else if (toLowerTrim(it.name) == "Spike Shield")
            {
                player.deflectPct = 25;
                narrate("Kamu menggunakan Spike Shield! 25% damage musuh akan dipantulkan.");
                it.quantity--;
            }
            else if (toLowerTrim(it.name) == "Smoke Orb")
            {
                narrate("Kamu melempar Smoke Orb! Kamu kabur 100% berhasil!");
                it.quantity--;
                enemy.hp = 0; // you see this , not dead but escaped
                return true;  // 
            }
            else
            {
...
void bossBattle_DraconicMaster(Player &player)
{
    ...
    DragonSari();
    Enemy boss = {"DRACONIC MASTER", 300, 300, 25, 10, 8, 200, 500, 0};
...
{
    system("cls");
    narrate("You emerge from the dark forest into a clearing...");
    narrate("The ground is covered in roots that pulse like veins.");
    narrate("Suddenly, the roots rise up, forming a massive, ancient tree-like entity.");
    typeText("FOREST GUARDIAN awakens...");
    Enemy boss = {"FOREST GUARDIAN", 400, 400, 30, 15, 6, 250, 500, 0};
    int poisonTurns = 0;
    ...
    while (player.hp > 0 && boss.hp > 0)
    {
        ...
        if (poisonTurns > 0)
        {
            cout << "Poison: " << poisonTurns << " turns remaining" << endl;
        }
        ...
        if (poisonTurns > 0)
        {
            int poisonDmg = 15;
            player.hp -= poisonDmg;
            narrate("Poison deals " + to_string(poisonDmg) + " damage!");
            poisonTurns--;
        }
        ...
        if (boss.hp > 0)
        {
            int move = rng(1, 4); // 1: Attack, 2: Poison, 3: Heal, 4: Summon

            if (move == 1)
            {
                narrate("\nFOREST GUARDIAN uses Root Whip!");
                int dmg = calculateDamage(boss.attack, player.defense);
                player.hp -= dmg;
                narrate("You take " + to_string(dmg) + " damage!");
            }
            else if (move == 2)
            {
                narrate("\nFOREST GUARDIAN releases Poison Spores!");
                poisonTurns = 3;
                narrate("You are poisoned for 3 turns!");
            }
            else if (move == 3)
            {
                narrate("\nFOREST GUARDIAN uses Nature's Embrace!");
                boss.hp += 30;
                if (boss.hp > boss.maxHP)
                    boss.hp = boss.maxHP;
                narrate("Boss recovers 30 HP!");
            }
            else if (move == 4)
            {
                narrate("\nFOREST GUARDIAN summons Thorn Vines!");
                int extraDmg = calculateDamage(boss.attack + 10, player.defense);
                player.hp -= extraDmg;
                narrate("Thorn Vines deal " + to_string(extraDmg) + " extra damage!");
            }
        }
    ...
    {
        narrate("\nYou died...");
        narrate("BAD ENDING — Entombed in the Forest");
        narrate("The roots entwine your body, becoming one with the forest.");
        pressEnter();
        narrate("???: Rest in peace, adventurer.");
        pressEnter();
        return;
    }

    // Victory
    narrate("\nYou defeated FOREST GUARDIAN!");
    typeText("The guardian crumbles into dust, revealing a path forward.");
    addItem(player.inv, "Forest Essence", 1, "Boosts defense by 10 permanently.");
    player.defense += 10;
    narrate("You gain Forest Essence! Defense +10!");
    narrate("\n=== VICTORY — The Forest Bows ===");
    pressEnter();
}
